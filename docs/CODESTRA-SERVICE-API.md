# Codestra service API contract: Node Exporter

This repository owns the **host-metrics-operational-evidence-authority** for the Codestra observability, analytics, telemetry, and secrets suite.

## Communication rule

Node Exporter keeps its native API and protocol. The shared Codestra control plane in `appolon1908-hue/Codestra-Telemetry` performs only sanitized health, readiness, contract, topology, and immutable-release read-back. It never proxies native query bodies, ingestion, alert delivery, dashboard mutations, secret values, or credential issuance.
...
|---|---|---|---|---|
| `prometheus` | outbound | `metrics` | `prometheus-scrape` | publish bounded host metrics |
...
The control plane reads source revision and image digest only from deployment environment variables. A valid release requires a 40-character Git SHA and `sha256:<64 lowercase hex>` image digest. This source change does not deploy the service, activate a scrape, expose the exporter, issue credentials, or enable any business, communications, financial, or trading mutation.
